package com.portal.dto;

public class DocumentoEntEncabezadoDTO {

	private Long cod_movimiento;
	private String cod_periodo;
	private String doc_tipo;
	private String doc_serie;
	private Long doc_numero;
	private String doc_fecha;
	private String doc_estado;
	private Long cod_empresa;
	private Long cod_tipo_movimiento;
	private Long cod_proveedor;
	private Double doc_monto_descuento;
	private Double doc_monto_gravado;
	private Double doc_monto_iva;
	private Double doc_monto_total;
	private String proveedor;
	private String tipo_movimiento;
	private String nombre_empresa;

	

	public Long getCod_movimiento() {
		return cod_movimiento;
	}

	public void setCod_movimiento(Long cod_movimiento) {
		this.cod_movimiento = cod_movimiento;
	}

	public String getDoc_tipo() {
		return doc_tipo;
	}

	public void setDoc_tipo(String doc_tipo) {
		this.doc_tipo = doc_tipo;
	}

	public String getDoc_serie() {
		return doc_serie;
	}

	public void setDoc_serie(String doc_serie) {
		this.doc_serie = doc_serie;
	}

	public Long getDoc_numero() {
		return doc_numero;
	}

	public void setDoc_numero(Long doc_numero) {
		this.doc_numero = doc_numero;
	}

	public String getDoc_fecha() {
		return doc_fecha;
	}

	public void setDoc_fecha(String doc_fecha) {
		this.doc_fecha = doc_fecha;
	}

	public String getDoc_estado() {
		return doc_estado;
	}

	public void setDoc_estado(String doc_estado) {
		this.doc_estado = doc_estado;
	}

	public Long getCod_empresa() {
		return cod_empresa;
	}

	public void setCod_empresa(Long cod_empresa) {
		this.cod_empresa = cod_empresa;
	}

	public Long getCod_tipo_movimiento() {
		return cod_tipo_movimiento;
	}

	public void setCod_tipo_movimiento(Long cod_tipo_movimiento) {
		this.cod_tipo_movimiento = cod_tipo_movimiento;
	}

	public Long getCod_proveedor() {
		return cod_proveedor;
	}

	public void setCod_proveedor(Long cod_proveedor) {
		this.cod_proveedor = cod_proveedor;
	}

	public Double getDoc_monto_descuento() {
		return doc_monto_descuento;
	}

	public void setDoc_monto_descuento(Double doc_monto_descuento) {
		this.doc_monto_descuento = doc_monto_descuento;
	}

	public Double getDoc_monto_gravado() {
		return doc_monto_gravado;
	}

	public void setDoc_monto_gravado(Double doc_monto_gravado) {
		this.doc_monto_gravado = doc_monto_gravado;
	}

	public Double getDoc_monto_iva() {
		return doc_monto_iva;
	}

	public void setDoc_monto_iva(Double doc_monto_iva) {
		this.doc_monto_iva = doc_monto_iva;
	}

	public Double getDoc_monto_total() {
		return doc_monto_total;
	}

	public void setDoc_monto_total(Double doc_monto_total) {
		this.doc_monto_total = doc_monto_total;
	}

	public String getProveedor() {
		return proveedor;
	}

	public void setProveedor(String proveedor) {
		this.proveedor = proveedor;
	}

	public String getTipo_movimiento() {
		return tipo_movimiento;
	}

	public void setTipo_movimiento(String tipo_movimiento) {
		this.tipo_movimiento = tipo_movimiento;
	}

	public String getCod_periodo() {
		return cod_periodo;
	}

	public void setCod_periodo(String cod_periodo) {
		this.cod_periodo = cod_periodo;
	}

	public String getNombre_empresa() {
		return nombre_empresa;
	}

	public void setNombre_empresa(String nombre_empresa) {
		this.nombre_empresa = nombre_empresa;
	}


	

}
